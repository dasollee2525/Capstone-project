...
def infer_2stage(gen, encode_loader_cho, encode_loader_jung, encode_loader_jong, decode_loader, reset_memory=True):
    print("INFER 2STAGE")
    ...
    for style_ids, style_comp_ids, style_imgs in encode_loader_cho:

        style_ids = style_ids.cuda()
        style_comp_ids = style_comp_ids.cuda()
        style_imgs = style_imgs.cuda()

        gen.encode_write("cho",style_ids, style_comp_ids, style_imgs, reset_dynamic_memory=False)

    for style_ids, style_comp_ids, style_imgs in encode_loader_jung:
        style_ids = style_ids.cuda()
        style_comp_ids = style_comp_ids.cuda()
        style_imgs = style_imgs.cuda()

        gen.encode_write("jung",style_ids, style_comp_ids, style_imgs, reset_dynamic_memory=False)

    for style_ids, style_comp_ids, style_imgs in encode_loader_jong:
        style_ids = style_ids.cuda()
        style_comp_ids = style_comp_ids.cuda()
        style_imgs = style_imgs.cuda()

        gen.encode_write("jong",style_ids, style_comp_ids, style_imgs, reset_dynamic_memory=False)
    print("=======================Encoding Done==================================Encoding Done==========================")
    # stage 2. decode
    outs = []
