...
class MACore(nn.Module):
    ...
    def encode_write(self, position, style_ids, comp_ids, style_imgs, reset_dynamic_memory=True):
        ...
        comp_feats = feats[-1] ## 마지막 껏만 넣기
        skips = self.component_encoder.filter_skips(feats)  # filter skip features
        self.memory.write(position,style_ids, comp_ids, comp_feats)

        if hasattr(self, 'skip_memory'):
            self.skip_memory.write(position,style_ids, comp_ids, skips[0])
