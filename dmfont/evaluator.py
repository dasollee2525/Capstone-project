...
class Evaluator:
    ...
    @torch_eval
    def handwritten_validation_2stage(self, gen, step, fonts, style_chars, target_chars, comparable=False, save_dir=None, tag='hw_validation_2stage'):
      ...
      font_jung = fonts[1]
      font_jong = fonts[2]
      font_name = fonts[0][0] + fonts[1][0] + fonts[2][0]

      encode_loader_cho = get_val_encode_loader(
          self.data, font_cho, style_chars, self.language, self.transform)

      encode_loader_jung = get_val_encode_loader(
          self.data, font_jung, style_chars, self.language, self.transform)

      encode_loader_jong = get_val_encode_loader(
          self.data, font_jong, style_chars, self.language, self.transform)
      ...
      if save_dir:
          for char, glyph in zip(target_chars, out):
              uni = "".join([f'{ord(each):04X}' for each in char])
              path = save_dir / "{}_{}.png".format("font_name", uni)
